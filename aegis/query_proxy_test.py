...
CASES = [
    ...
    ("q: unbounded scan gets a row cap too",
     "select from trade", "rewrite", "select[1000000]"),
    ("q: has date, still gets a row cap",
     "select sym,price from quote where date=2024.01.01", "rewrite", "select[1000000]"),
    ("q: already bounded -> allow",
    ...
    ("q: non-partitioned ref table, capped",
     "select from ref", "rewrite", "select[1000000]"),
