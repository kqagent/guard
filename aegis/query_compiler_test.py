...
CASES = [
    ...
    ("oversized limit is capped (result-row sublist, not scan)", {"table": "trade", "columns": ["sym"], "date": D, "limit": 999999999}, "1000000 sublist"),
    ("aggregation is NOT scan-capped (correctness at scale): count over full partition",
     {"table": "trade", "aggs": [{"fn": "count", "as": "n"}], "date": D}, "1000000 sublist (select n:count i from trade"),
    ("raw-row query is result-capped via sublist",
     {"table": "trade", "columns": ["sym", "price"], "date": D}, "1000000 sublist (select sym, price from trade"),
    ...
    ("TWAP — weighted aggregate over deltas (time-weighted)",
     {"table": "trade", "select": [{"as": "twap", "expr": {"agg": "wavg", "weight": {"win": "deltas", "arg": {"col": "time"}}, "arg": {"col": "price"}}}], "date": D}, "(deltas time) wavg (price)"),
    ("compute-over-join (effective spread)",
     {"join": {"type": "asof", "on": ["sym", "time"],
               "left": {"table": "trade", "columns": ["sym", "time", "price"], "date": D},
               "right": {"table": "quote", "columns": ["sym", "time", "bid", "ask"], "date": D},
               "select": [{"as": "eff", "expr": {"op": "sub", "args": [{"col": "price"}, {"op": "div", "args": [{"op": "add", "args": [{"col": "bid"}, {"col": "ask"}]}, {"lit": 2}]}]}}]}}, "eff:(price-((bid+ask)%2))"),
    ("cross-table left join (trade vs quote count)",
     {"join": {"type": "left", "on": ["sym"],
               "left": {"table": "trade", "aggs": [{"fn": "count", "as": "nt"}], "by": ["sym"], "date": D},
               "right": {"table": "quote", "aggs": [{"fn": "count", "as": "nq"}], "by": ["sym"], "date": D}}}, ") lj ("),
    # abuse for the new slots
    ("expr wavg without weight -> reject",
     {"table": "trade", "select": [{"as": "x", "expr": {"agg": "wavg", "arg": {"col": "price"}}}], "date": D}, "REJECT"),
    ("left join without by-key -> reject",
     {"join": {"type": "left", "on": ["sym"],
               "left": {"table": "trade", "columns": ["sym"], "date": D},
               "right": {"table": "quote", "columns": ["sym"], "date": D}}}, "REJECT"),
    ("compute-over-join refs non-union column -> reject",
     {"join": {"type": "asof", "on": ["sym", "time"],
               "left": {"table": "trade", "columns": ["sym", "time"], "date": D},
               "right": {"table": "quote", "columns": ["sym", "time"], "date": D},
               "select": [{"as": "x", "expr": {"col": "password"}}]}}, "REJECT"),
